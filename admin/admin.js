...
class AdminPanel {
    ...
    async importSpotifyTrack() {
        ...
        // Ensure hidden input for imported artwork URL exists
        let importedArtworkInput = document.getElementById('importedArtworkUrl');
        if (!importedArtworkInput) {
            importedArtworkInput = document.createElement('input');
            importedArtworkInput.type = 'hidden';
            importedArtworkInput.id = 'importedArtworkUrl';
            importedArtworkInput.name = 'importedArtworkUrl';
            document.getElementById('audioUploadForm')?.appendChild(importedArtworkInput);
        }
        ...
        if (importedArtworkInput && this.selectedSpotifyTrack.artwork) {
            importedArtworkInput.value = this.selectedSpotifyTrack.artwork;
        }
    ...
    async handleExternalTrack() {
        ...
        try {
            ...
            if (audioUrlInput) {
                audioUrlInput.value = normalizedUrl;
                audioUrlInput.dispatchEvent(new Event('input', { bubbles: true }));
            }
            ...
            if (artistInput) {
                try {
                    const artists = await window.fetchArtists();
                    const incoming = String(artist || '').toLowerCase();
                    const matchingArtist = (artists || []).find(a => String(a.name || '').toLowerCase() === incoming);
                    artistInput.value = matchingArtist ? matchingArtist.id : '__add_new__';
                } catch (_) {
                    artistInput.value = '__add_new__';
                }
            ...
            if (descriptionInput) {
                const platformLabel = platform === 'apple-music' ? 'Apple Music' : (platform || 'External Platform');
                descriptionInput.value = `Imported from ${platformLabel}: ${normalizedUrl}`;
            }
            ...
            if (importedArtworkInput && artwork) {
                importedArtworkInput.value = artwork;
            }

            this.switchUploadMethod('upload');
            this.resetExternalForm();
            this.showNotification('Imported data moved to Audio Link form. Please complete missing information and save.', 'success');
        } catch (error) {
