...
class AdminPanel {
    ...
    async importSpotifyTrack() {
        ...
        const desiredArtistValue = artistInput ? String(artistInput.value || '') : '';
        ...
        // Re-apply artist selection after the UI switches (other scripts may re-render the select options)
        if (desiredArtistValue) {
            setTimeout(() => {
                const sel = document.getElementById('trackArtist');
                if (sel) sel.value = desiredArtistValue;
            }, 400);
        }

        if (desiredArtistValue === '__add_new__') {
            const name = String(this.selectedSpotifyTrack.artist || '').trim();
            const msg = `Artist "${name || 'Unknown'}" was not found in your Artists list. Add this artist now?`;
            let ok = false;
            if (window.notifications && window.notifications.confirm) {
                ok = await window.notifications.confirm(msg, 'Add Artist', 'info');
            } else {
                ok = confirm(msg);
            }
            if (ok) {
    ...
    async handleExternalTrack() {
        ...
        try {
            ...
            const desiredArtistValue = artistSelect ? String(artistSelect.value || '') : '';
